...
- Define a `POST` block inside of the `/baked_goods` route that creates a new
...
- Define a `DELETE` block inside of the `/baked_goods/<int:id>` route that
...
![POST request with form data for a bakery's name](
